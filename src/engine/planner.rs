...
fn looking_glass_step(install_mode: crate::vm::LookingGlassInstallMode) -> PlannedStep {
    match install_mode {
        crate::vm::LookingGlassInstallMode::Manual => PlannedStep {
            ...
            title: "Surface Looking Glass install instructions".to_string(),
            summary: "Print distro-specific Looking Glass dependencies and the manual install steps. No host files are written; the user is responsible for installing the client.".to_string(),
            risk: StepRisk::ReadOnly,
            privilege: PrivilegeNeed::User,
            state: StepState::Pending,
            touches: Vec::new(),
            commands: Vec::new(),
            verification: "Read-only step; no verification beyond confirming the user acknowledged the instructions.".to_string(),
        ...
        crate::vm::LookingGlassInstallMode::AutoBuild => PlannedStep {
            kind: StepKind::LookingGlassInstall,
            title: "Build Looking Glass from source after explicit consent".to_string(),
            summary: "After explicit user confirmation, download the stable Looking Glass source, verify extraction, and compile locally. Writes the IVSHMEM tmpfiles.d snippet only after the build succeeds. No telemetry; no remote API calls.".to_string(),
            risk: StepRisk::Medium,
            privilege: PrivilegeNeed::User,
            state: StepState::Pending,
            touches: vec![PathBuf::from("/etc/tmpfiles.d/10-looking-glass.conf")],
            commands: Vec::new(),
            verification: "Confirm the Looking Glass client binary exists and the IVSHMEM tmpfile is writable by the invoking user.".to_string(),
            rollback: "Remove generated tmpfiles config and any local build artifacts. Restore prior tmpfiles config from the snapshot manifest.".to_string(),
            requires_reboot: false,
            requires_confirmation: true,
        },
    }
...
fn verify_step() -> PlannedStep {
    PlannedStep {
        ...
        summary: "Re-scan IOMMU groups, vfio-pci binding, libvirt domain visibility, and (if enabled) Looking Glass IVSHMEM availability.".to_string(),
        risk: StepRisk::ReadOnly,
        privilege: PrivilegeNeed::User,
        state: StepState::Pending,
        touches: Vec::new(),
